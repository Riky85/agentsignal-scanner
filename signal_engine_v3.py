#!/usr/bin/env python3
"""signal_engine_v3.py — v3.3
CONFIRMED working fields only.
Workaround: company_size="scanned"/"unreachable", industry=categoria opportunità
"""
...
stats={"scanned":0,"unreachable":0,"errors":0,"cycle":0,
       "current":"","queue":0,"good":0,"last_qc":"never","qc":{}}
...
def scan(rec):
    ...
    ev={"Robotics & Cobot":ro_h[:4],"MES/Digital Factory":m_h[:4],
        "Automation Upgrade":a_h[:4],"High Buying Intent":i_h[:4],"Low Signal":[]}
    ...
    try:
        ...
        while True:
            b=requests.get(f"{BASE}?limit=500&skip={skip}&fields=buying_intent_score,"
                           ...
                           f"mes_opportunity_score,name,company_size",
                           headers=HDRS,timeout=20).json()
            if not isinstance(b,list) or not b:break
            for x in b:
                bi=x.get("buying_intent_score")
                cs=x.get("company_size") or ""
                if bi is None and "scanned" not in cs:pending+=1;continue
                total+=1
...
def load_pending(limit=100):
    """Non scansionato = buying_intent_score IS NULL AND company_size != 'scanned'"""
    results=[];skip=0
    while len(results)<limit:
        try:
            b=requests.get(f"{BASE}?limit=200&skip={skip}"
                           f"&fields=id,name,domain,website_url,employee_count,"
                           f"buying_intent_score,country,description,company_size",
                           headers=HDRS,timeout=20).json()
        except Exception as e:log.warning(f"load: {e}");break
        if not isinstance(b,list) or not b:break
        for r in b:
            bi=r.get("buying_intent_score")
            cs=(r.get("company_size") or "")
            if bi is None and "scanned" not in cs:
                results.append(r)
...
log.info("=== SIGNAL ENGINE v3.3 ===")
log.info("Workaround: company_size='scanned'/'unreachable'")
log.info("industry = categoria opportunità")
log.info("QC ogni 50 scan, scansione continua")
...
while True:
    ...
    if not batch:
        log.info("DB completo. QC finale + pausa 2h.")
        quality_check();time.sleep(7200);continue
    ...
    for rec in batch:
        ...
        try:
            ...
            payload={
                ...
                "company_size":               "scanned",
                # industry aggiornato con categoria (solo se top non è Low Signal)
                "industry":                   result["_top"] if result["_top"]!="Low Signal"
                                              else (rec.get("industry") or "Manufacturing"),
            }
            # description = evidenze (solo se campo vuoto)
            if not (rec.get("description") or "").strip() and result["_top"]!="Low Signal":
                payload["description"]=f"{result['_solution']}"[:400]
    ...
    log.info(f"[C{stats['cycle']}] scanned={stats['scanned']} "
             f"unreachable={stats['unreachable']} good={stats['good']} err={stats['errors']}")
