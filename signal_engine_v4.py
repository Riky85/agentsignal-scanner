#!/usr/bin/env python3
"""signal_engine_v4.py — scrive campi corretti + crea record IndustrialSignal reali
per popolare il pannello "Why Now" del frontend.
"""
...
stats={"scanned":0,"unreachable":0,"errors":0,"cycle":0,
       "current":"","queue":0,"good":0,"signals_created":0,"last_qc":"never","qc":{}}
...
MES_KW=[
    ...
    "jumeau numérique","traçabilité de production",
]
INTENT_KW=[
...
def scan(rec):
    ...
    text="";pages_ok=0;source_url=base_url
    for url in [base_url,f"{base_url}/products",f"{base_url}/prodotti",
                f"{base_url}/solutions",f"{base_url}/technology",
                f"{base_url}/careers",f"{base_url}/jobs",f"{base_url}/about"]:
        t=fetch(url)
        if t:
            text+=" "+t;pages_ok+=1
        if len(text)>28000:break
    if len(text.strip())<300:
        return {"_status":"unreachable"}
    if BLACKLIST.search(text[:3000]):
        return {"_status":"blacklisted"}
    ...
    return {
        "_status":"scanned","_top":top,"_solution":solution,"_pages":pages_ok,
        "_evidence":ev,"_source_url":source_url,
        "automation_readiness_score":auto_s,
        "robotics_opportunity_score":robot_s,
        "mes_opportunity_score":mes_s,
        "buying_intent_score":int_s,
        "amr_agv_opportunity_score":round((robot_s+auto_s)/2),
        "estimated_deal_value_min":dmin,
        "estimated_deal_value_max":dmax,
    }

def create_signals(rec, result):
    """Crea record IndustrialSignal reali per il pannello Why Now."""
    created = 0
    scores = {"Robotics & Cobot":result["robotics_opportunity_score"],
              "MES/Digital Factory":result["mes_opportunity_score"],
              "Automation Upgrade":result["automation_readiness_score"],
              "High Buying Intent":result["buying_intent_score"]}
    now = time.strftime("%Y-%m-%dT%H:%M:%S")
    for cat, score in scores.items():
        if score < 15: continue
        evidence_kw = result["_evidence"].get(cat, [])
        if not evidence_kw: continue
        payload = {
            "company_id": rec["id"],
            "company_name": rec.get("name",""),
            "company_domain": rec.get("domain",""),
            "signal_category": CATEGORY_MAP.get(cat, "growth_buying_intent"),
            "signal_type": evidence_kw[0][:60],
            "source_url": result.get("_source_url",""),
            "evidence_text": f"Rilevate {len(evidence_kw)} evidenze tecniche: {', '.join(evidence_kw)}"[:500],
...
def quality_check():
    log.info("── QC ──")
    try:
        skip=0;total=0;good=0;zero=0;pending=0;top10=[]
        while True:
            b=requests.get(f"{BASE}?limit=500&skip={skip}&fields=buying_intent_score,"
                           ...
                           f"mes_opportunity_score,name,scan_status",
                           headers=HDRS,timeout=20).json()
            if not isinstance(b,list) or not b:break
            for x in b:
                bi=x.get("buying_intent_score")
                ss=x.get("scan_status") or ""
                if ss=="pending":pending+=1;continue
                total+=1
                ro=x.get("robotics_opportunity_score") or 0
                au=x.get("automation_readiness_score") or 0
                me=x.get("mes_opportunity_score") or 0
                best=max(bi or 0,ro,au,me)
                if best==0:zero+=1
                elif best>=20:good+=1
                if (bi or 0)>=30:top10.append((bi,ro,au,me,x.get("name","?")))
            skip+=500
            if len(b)<500:break
        rate=round(good/total*100,1) if total else 0
        top10.sort(reverse=True)
        log.info(f"  Total:{total+pending} | Scansionati:{total} | Pending:{pending}")
        log.info(f"  Con segnale≥20: {good} ({rate}%) | Zero: {zero}")
        for bi,ro,au,me,nm in top10[:5]:
            log.info(f"    {nm[:40]:42} I={bi:.0f} R={ro:.0f} A={au:.0f} M={me:.0f}")
        stats["qc"]={"total":total,"pending":pending,"good":good,"zero":zero,"rate":rate}
        stats["last_qc"]=time.strftime("%H:%M:%S")
        log.info("── END QC ──")
    except Exception as e:log.warning(f"qc: {e}")
    ...
    while len(results)<limit:
        try:
            b=requests.get(f"{BASE}?limit=200&skip={skip}"
                           ...
                           f"country,description,scan_status",
                           headers=HDRS,timeout=20).json()
        except Exception as e:log.warning(f"load: {e}");break
        if not isinstance(b,list) or not b:break
        for r in b:
...
log.info("=== SIGNAL ENGINE v4.0 ===")
log.info("Scrive scan_status/scanned/pipeline_notes/top_opportunity corretti")
log.info("+ crea record IndustrialSignal reali per pannello Why Now")
...
while True:
    ...
    if not batch:
        log.info("DB completo. QC finale + pausa 1h.")
        quality_check();time.sleep(3600);continue
    batch.sort(key=lambda x:(0 if (x.get("country") or "").upper().strip() in PRIORITY else 1))
    log.info(f"[C{stats['cycle']}] Batch {len(batch)}")
    for rec in batch:
        ...
        try:
            ...
            status=result.get("_status","unreachable")
            if status in("unreachable","blacklisted"):
                r=requests.put(f"{BASE}/{rec['id']}",
                    json={"buying_intent_score":0,"automation_readiness_score":0,
                          ...
                          "scan_status":status,"scanned":True,"last_scan_date":now},
                    headers=HDRS,timeout=10)
                if r.status_code in(200,201,204):
                    stats["unreachable"]+=1;sqc+=1
                    log.info(f"  ⚠️  {name}: {status}")
                continue
            payload={
                ...
                "estimated_deal_min":         result["estimated_deal_value_min"],
                "estimated_deal_max":         result["estimated_deal_value_max"],
            ...
            if r.status_code in(200,201,204):
                stats["scanned"]+=1;sqc+=1
                n_sig = create_signals(rec, result)
                ...
                if max(bi,ro,au,me)>=20:stats["good"]+=1
                log.info(f"  ✅ {name:40} "
                         f"A={au:3.0f} R={ro:3.0f} M={me:3.0f} I={bi:3.0f} "
                         f"sig={n_sig} → {result['_top'][:30]}")
            else:
                stats["errors"]+=1
                log.warning(f"  ❌ {domain}: HTTP {r.status_code}")
        except Exception as e:
            stats["errors"]+=1;log.warning(f"  ❌ {domain}: {e}")
        if sqc>=50:sqc=0;quality_check()
        time.sleep(1.0)
    log.info(f"[C{stats['cycle']}] scanned={stats['scanned']} "
             f"unreachable={stats['unreachable']} good={stats['good']} "
